...
print(f"Starting NIVARA data ingestion pipeline from {RAW_DIR}...")

# Copy GeoJSON to public and datasets/gis
geojson_src = os.path.join(RAW_DIR, "07_Cadastral_Prototype.geojson")
if os.path.exists(geojson_src):
    shutil.copyfile(geojson_src, os.path.join(FRONTEND_PUBLIC, "cadastral_parcels.geojson"))
    shutil.copyfile(geojson_src, os.path.join(PROJECT_ROOT, "datasets", "gis", "cadastral_parcels.geojson"))
    shutil.copyfile(geojson_src, os.path.join(PROJECT_ROOT, "gis", "geojson", "cadastral_parcels.geojson"))
    print(f"Copied {geojson_src} -> frontend/public & datasets/gis")
...
out_frontend = os.path.join(FRONTEND_PUBLIC, "data.json")
out_datasets = os.path.join(DATASETS_PROCESSED, "data.json")
...
print(f"\nSuccessfully generated {out_frontend} ({os.path.getsize(out_frontend) / 1024:.1f} KB)")
print(f"Successfully generated {out_datasets} ({os.path.getsize(out_datasets) / 1024:.1f} KB)")
